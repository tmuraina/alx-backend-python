...
class TestGithubOrgClient(unittest.TestCase):
    ...
    def test_public_repos(self, mock_get_json):
        """Test that public_repos returns expected repo list and calls methods once."""
        test_repos_payload = [
            {"name": "repo1"},
            {"name": "repo2"},
            {"name": "repo3"}
        ...
        with patch('client.GithubOrgClient._public_repos_url',
                   new_callable=lambda: property(lambda self: "https://api.github.com/orgs/google/repos")) as mock_public_repos_url:
            client = GithubOrgClient("google")
